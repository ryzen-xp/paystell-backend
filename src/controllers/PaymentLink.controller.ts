...
export class PaymentLinkController {
  ...
  async createPaymentLink(
    ...
    try {
      ...
      console.log(
        "[PaymentLinkController] Successfully created payment link:",
        paymentLink,
      );
      return res.status(201).json(paymentLink);
    } catch (error) {
      console.error(
        "[PaymentLinkController] Error creating payment link:",
        error,
      );
  ...
  async getPaymentLinkById(req: Request, res: Response): Promise<Response> {
    try {
      console.log(
        "[PaymentLinkController] Getting payment link by id:",
        req.params.id,
      );
      ...
      if (!paymentLink) {
        console.log(
          "[PaymentLinkController] Payment link not found:",
          req.params.id,
        );
      ...
      console.error(
        "[PaymentLinkController] Error getting payment link:",
        error,
      );
  ...
  async getPaymentLinksByUserId(
    ...
    try {
      ...
      console.error(
        "[PaymentLinkController] Error getting payment links by user:",
        error,
      );
      return res.status(500).json({ message: (error as Error).message });
    }
  ...
  async updatePaymentLink(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (!updatedPaymentLink) {
        console.log(
          "[PaymentLinkController] Payment link not found for update:",
          req.params.id,
        );
        return res.status(404).json({ message: "PaymentLink not found" });
      }
      ...
      console.error(
        "[PaymentLinkController] Error updating payment link:",
        error,
      );
  ...
  async deletePaymentLink(req: Request, res: Response): Promise<Response> {
    try {
      console.log(
        "[PaymentLinkController] Deleting payment link:",
        req.params.id,
      );
      ...
      if (!success) {
        console.log(
          "[PaymentLinkController] Payment link not found for deletion:",
          req.params.id,
        );
        return res.status(404).json({ message: "PaymentLink not found" });
      }
      ...
      console.error(
        "[PaymentLinkController] Error deleting payment link:",
        error,
      );
      return res.status(500).json({ message: (error as Error).message });
    }
  ...
  async softDeletePaymentLink(req: Request, res: Response): Promise<Response> {
    try {
      console.log(
        "[PaymentLinkController] Soft deleting payment link:",
        req.params.id,
      );
      ...
      if (!success) {
        console.log(
          "[PaymentLinkController] Payment link not found for soft deletion:",
          req.params.id,
        );
        return res.status(404).json({ message: "PaymentLink not found" });
      }
      ...
      console.error(
        "[PaymentLinkController] Error soft deleting payment link:",
        error,
      );
      return res.status(500).json({ message: (error as Error).message });
    }
